...
public class MedicalFileS extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			this.getServletContext().getRequestDispatcher("/medicalFile.jsp").forward(request, response);
		...
		if (request.getParameter("id_appointment") != null) {
			...
			try {
				...
				if (consultationDao.getConsultationById(id_consultation).getPrescription() != null) {
					int id_prescription = consultationDao.getConsultationById(id_consultation).getPrescription()
							.getId_prescription();
			...
			try {
				boolean isDelete2 = resultatDao.deleteResultatById(id_resultat);
				doGet(request, response);
			} catch (SQLException e) {
